...
const musicData = [
  {
    ...
    quotes: ["Look, if you had one shot...", "Success is my only option, failure's not.", "I am whatever you say I am."],
  ...
  },
  {
    ...
    image: 'https://images.unsplash.com/photo-1516280440502-65f536c97646?q=80&w=500&auto=format&fit=crop',
    quotes: ["In your life, perhaps I am merely a fleeting spark...", "OH YEAH!", "A hundred ways to live."],
  ...
  },
  {
    ...
    image: 'https://images.unsplash.com/photo-1493225457124-a1a2a5f5f9af?q=80&w=500&auto=format&fit=crop',
    quotes: ["Loving you is loving myself.", "You are the one and only.", "Forever love."],
...
const gameData = [
  {
    ...
    recommendation: 'Transforming the French Revolution and Notre Dame into an epic open world.',
    details: 'Despite its rocky launch, AC Unity remains the pinnacle of parkour and dense urban stealth in the franchise. The recreation of Paris is breathtaking.',
  ...
  },
  {
    ...
    recommendation: 'The story, music, and freedom of choice are off the charts.',
    details: 'Night City is a character of its own. With the Phantom Liberty expansion, this game solidified its place as one of the greatest RPGs of the decade.',
  ...
  },
  {
    ...
    recommendation: "Looks simple, but you won't stop playing—strategic depth is off the charts.",
    details: 'A deck-building roguelike that turns poker into a mesmerizing, combo-driven addiction. The joker synergies are endless.',
...
const MusicCard: React.FC<{ data: any, onPlay: (track: string) => void }> = ({ data, onPlay }) => {
  ...
  return (
    ...
    >
      ...
      >
        <img src={data.image} alt={data.artist} className="w-20 h-20 md:w-24 md:h-24 rounded-lg object-cover grayscale contrast-125 shrink-0" />
        <div className="flex-1 min-w-0">
          <h4 className="text-xl md:text-2xl font-black text-white uppercase tracking-wider truncate">{data.artist}</h4>
          <p className="text-[10px] md:text-xs text-white/60 mt-1 line-clamp-2 leading-tight">{data.bio}</p>
...
const GameCard: React.FC<{ data: any, onClick: () => void, globalGlitch: boolean }> = ({ data, onClick, globalGlitch }) => {
  const [isHovered, setIsHovered] = useState(false);
  ...
  return (
    ...
    >
      <div className="absolute inset-0 opacity-40 group-hover:opacity-60 transition-opacity duration-500">
        <img src={data.image} alt={data.title} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700 scale-105 group-hover:scale-100" />
      </div>
      ...
      <div className="relative z-20 p-4 md:p-6 flex flex-col h-full justify-end bg-gradient-to-t from-black via-black/80 to-transparent">
        <div className="text-[10px] font-mono text-primary mb-1 tracking-widest uppercase">{data.rank}</div>
        <h4 className="text-xl md:text-2xl font-black text-white uppercase tracking-tight leading-none mb-1">{data.title}</h4>
        <p className="text-xs md:text-sm text-white/80 font-medium mb-1 truncate">{data.subtitle}</p>
        <p className="text-[10px] text-white/50 line-clamp-1">{data.recommendation}</p>
      </div>
    </div>
  );
};

const NowPlayingOverlay = ({ track, onClose }: { track: string | null, onClose: () => void }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[9px] text-cyan-400 font-mono mb-1 uppercase tracking-wider">Opening Rocky's Realm... 🎧</p>
...
const GameModal = ({ game, onClose }: { game: any, onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
      ...
      >
        <button onClick={onClose} className="absolute top-4 right-4 z-20 w-8 h-8 bg-black/50 backdrop-blur-sm rounded-full flex items-center justify-center text-white hover:bg-white/20 transition-colors">
          <X size={18} />
        </button>
        ...
        <div className="p-6 md:p-8 relative z-10 -mt-10">
          <div className="text-xs font-mono text-primary mb-2 tracking-widest uppercase bg-black/50 inline-block px-2 py-1 rounded backdrop-blur-sm">{game.rank}</div>
          <h2 className="text-3xl md:text-4xl font-black text-white uppercase tracking-tight mb-2 leading-none">{game.title}</h2>
          <p className="text-base md:text-lg text-white/80 font-medium mb-6">{game.subtitle}</p>
...
export function GameMediaModule() {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row relative z-10">
        ...
        <div className="w-full lg:w-[45%] p-6 md:p-8 border-b lg:border-b-0 lg:border-r border-white/10 bg-gradient-to-br from-black to-[#0a0a0a]">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="font-mono text-xs uppercase tracking-[0.2em] font-bold text-white/80">Music Hall</h3>
          </div>
        ...
        <div className="w-full lg:w-[55%] p-6 md:p-8 bg-gradient-to-bl from-[#0a0a0a] to-black">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h3 className="font-mono text-xs uppercase tracking-[0.2em] font-bold text-white/80">Game Vault</h3>
            </div>
      ...
      <AnimatePresence>
        {playingTrack && (
          <NowPlayingOverlay track={playingTrack} onClose={() => setPlayingTrack(null)} />
        )}
      </AnimatePresence>
