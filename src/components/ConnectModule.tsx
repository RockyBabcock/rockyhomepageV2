...
import { Copy, CheckCircle2, Send, Twitter, Linkedin, Github, MessageSquare, ChevronDown, ChevronUp, Mail } from 'lucide-react';
import gsap from 'gsap';

const TAGS = [
  { label: 'Chat about Web3 / AI Projects', text: "Hi Rocky, your GitHub and website are really inspiring! I'd love to have an in-depth discussion with you regarding potential collaboration on AI projects / Web3..." },
  { label: 'Spurs Fans Assemble 👽', text: "Hey Rocky, I saw your 'Wemby' module—I'm practically becoming an 'Alien' fan myself! I wanted to chat with you about the Spurs' championship odds for next season..." },
  { label: 'Assassin’s Creed: Unity Enthusiasts', text: "Hey Rocky, AC Unity's parkour is unmatched. Let's talk about the best stealth routes and phantom blade tricks..." },
  { label: 'Looking for More Personal Website Inspiration', text: "Hi Rocky, your site is amazing! I'm looking for some inspiration for my own personal website and would love to hear about your design process..." },
  { label: 'Collaboration / Consultation', text: "Hi Rocky, I'm interested in collaborating on an upcoming project. Do you have some time to discuss potential synergies?" },
  { label: 'Just a Casual Chat', text: "Hi Rocky! I just stumbled upon your website while browsing—it looks really interesting! Thought I'd drop you a quick line..." }
];

const TEMPLATES = [
  { label: 'Collaboration Inquiry', text: "Hi Rocky, I'm interested in collaborating on a project related to Web3/AI. Let's discuss." },
  { label: 'Fan Message', text: "Hey, I’ve been following your work for a while! Love what you’ve done with the site, especially the Wemby Module." },
  { label: 'Feedback/Suggestions', text: "Hi Rocky, I wanted to share some feedback on your site. Love the modular design, but perhaps a few tweaks in the footer." }
];
...
export function ConnectModule() {
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      localStorage.setItem('connect_draft', JSON.stringify({ name, email, message }));
    }, 1000);
    ...
    >
      ...
      <div className="relative z-10 mb-10">
        ...
        <p className="text-gray-400 text-sm md:text-base max-w-2xl mb-4">
          Want to chat about Web3, AI, the Spurs, gaming, coding... or just vent about anything? Feel free to reach out anytime.
        </p>
      ...
      <div className="flex flex-col md:flex-row gap-10 relative z-10">
          ...
          {!isSuccess ? (
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-wider text-gray-500">Quick Topics</label>
                <div className="flex overflow-x-auto pb-2 gap-2 scrollbar-hide" style={{ scrollbarWidth: 'none' }}>
                  {TAGS.map((tag, i) => (
                    <button
                      key={i}
                      type="button"
                  ...
                  {showTemplates && (
                    ...
                    >
                      {TEMPLATES.map((tpl, i) => (
                        ...
                        >
                          <span className="font-bold text-white block mb-1">{tpl.label}</span>
                          {tpl.text}
                        </button>
                      ))}
                    </motion.div>
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="sendCopy" className="text-xs text-gray-400 cursor-pointer select-none">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2">
                <p className="text-[10px] text-gray-500 font-mono">
                  Used solely for replies; never for marketing or sharing your information.
                </p>
                <button 
                  type="submit"
            ...
            >
              ...
              {wembyEasterEgg && (
                <div className="mb-6 px-4 py-2 bg-purple-500/20 border border-purple-500/30 rounded-lg text-purple-300 text-sm">
                  Received! Bonus Reward: Your Wemby MVP prediction has been recorded 👽
                </div>
              )}
              <div className="flex flex-col gap-2 w-full max-w-xs">
                <button onClick={() => setIsSuccess(false)} className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-sm transition-colors">
        ...
        <div className="md:w-[42%] flex flex-col gap-8 border-t md:border-t-0 md:border-l border-white/10 pt-8 md:pt-0 md:pl-10">
          ...
          <div>
            <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-4">Direct Email</h4>
                ...
                >
                  ...
                  <AnimatePresence>
                    {showEmailEasterEgg && (
                      ...
                      >
                        ProtonMail: Security First! But actually, what I really want to talk about is... what’s your favorite Balatro deck? 😂
                      </motion.div>
                    )}
                  </AnimatePresence>
                </div>
                <div>
                  <p className="text-sm font-medium text-white">braverocky@proton.me</p>
                  <p className="text-xs text-gray-500">PGP Key available on request</p>
                </div>
              </div>
