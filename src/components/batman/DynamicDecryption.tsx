...
export function DynamicDecryption({ 
  ...
  const handleVerify = (e?: React.FormEvent) => {
    ...
    if (puzzleContext && attempt.toLowerCase() === puzzleContext.answer.toLowerCase()) {
      setUnlocked(true);
      setShowPuzzle(false);
    ...
    <span className="relative inline-block mx-1">
       {!showPuzzle ? (
         ...
         >
            ...
            <div className="text-xs text-white/80 mb-3">{puzzleContext?.question || "ENTER OVERRIDE CODE"}</div>
            <form onSubmit={handleVerify} className="flex gap-2">
              ...
              />
              <button type="submit" className="text-[10px] bg-red-500 text-black px-2 hover:bg-white transition-colors">VRY</button>
              <button type="button" onClick={() => setShowPuzzle(false)} className="text-[10px] border border-white/20 text-white/50 px-2 hover:bg-white/10 transition-colors">X</button>
            </form>
         </motion.div>
       )}
    </span>
