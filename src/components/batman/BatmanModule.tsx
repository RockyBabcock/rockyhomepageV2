...
export function BatmanModule() {
  ...
  const [roomView, setRoomView] = useState<"cabinets" | "map" | "comparator">("cabinets");
        ...
        {stage === "room" && (
          ...
          >
            <div className="absolute top-16 left-0 right-0 z-50 flex justify-center gap-4 p-4 pointer-events-auto">
               <button onClick={() => setRoomView('cabinets')} className={`px-4 py-2 border text-[10px] uppercase tracking-widest transition-colors ${roomView === 'cabinets' ? 'bg-[#d4af37] text-black border-[#d4af37]' : 'bg-black/50 text-[#d4af37]/50 border-[#d4af37]/30 hover:border-[#d4af37]'}`}>Archives</button>
               <button onClick={() => setRoomView('map')} className={`px-4 py-2 border text-[10px] uppercase tracking-widest transition-colors ${roomView === 'map' ? 'bg-[#d4af37] text-black border-[#d4af37]' : 'bg-black/50 text-[#d4af37]/50 border-[#d4af37]/30 hover:border-[#d4af37]'}`}>City Map</button>
               <button onClick={() => setRoomView('comparator')} className={`px-4 py-2 border text-[10px] uppercase tracking-widest transition-colors ${roomView === 'comparator' ? 'bg-[#00ff41] text-black border-[#00ff41]' : 'bg-black/50 text-[#00ff41]/50 border-[#00ff41]/30 hover:border-[#00ff41]'}`}>Analysis</button>
            </div>

            <div className="flex-1 relative">
               <AnimatePresence mode="wait">
                 {roomView === 'cabinets' && (
                   <motion.div key="cabinets" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0">
                 ...
                 {roomView === 'comparator' && (
                   <motion.div key="comparator" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0">
