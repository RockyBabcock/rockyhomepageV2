...
const FRUITS = [
  { id: 'chess', icon: '♟️', label: 'Chess', color: 'bg-gray-800', link: '#Chess' },
  { id: 'basketball', icon: '🏀', label: 'Hoops', color: 'bg-orange-500', link: '#Basketball' },
  { id: 'vinyl', icon: '💿', label: 'Media', color: 'bg-purple-500', link: '#GameMedia' },
  { id: 'ac', icon: '🦅', label: 'AC Unity', color: 'bg-red-600', link: '#Timeline' },
  { id: 'github', icon: <Github size={16} />, label: 'GitHub', color: 'bg-black', link: '#GitHub' },
];

// --- Subcomponents ---

const TreeSVG = ({ stageIndex, isJoker, isKnight, isGotham, isWemby, theme }: any) => {
  ...
  return (
    <div ref={containerRef} className={`relative w-full h-full flex items-end justify-center pb-10 transition-transform duration-1000 ${isGotham ? 'rotate-180' : ''}`}>
      <svg width="400" height="500" viewBox="0 0 400 500" className="overflow-visible">
        {/* Shadow */}
        <ellipse ref={shadowRef} cx="200" cy="480" rx="50" ry="15" fill="rgba(0,0,0,0.3)" />
        
        {/* Trunk */}
        <g ref={trunkRef}>
          <path d="M180,480 Q200,250 200,200 Q200,250 220,480 Z" fill="#5D4037" />
          {/* Face */}
          {stageIndex >= 2 && (
            <g transform="translate(190, 400) scale(0.5)">
              <circle cx="5" cy="0" r="3" fill="#3E2723" />
              <circle cx="25" cy="0" r="3" fill="#3E2723" />
              <path d="M10,10 Q15,15 20,10" stroke="#3E2723" strokeWidth="2" fill="none" strokeLinecap="round" />
            </g>
          )}
        </g>
        ...
        <g ref={branchesRef} opacity="0">
          <path d="M200,350 Q120,280 90,250" stroke="#5D4037" strokeWidth="12" strokeLinecap="round" fill="none" />
          <path d="M200,300 Q280,230 310,200" stroke="#5D4037" strokeWidth="10" strokeLinecap="round" fill="none" />
          <path d="M200,250 Q140,180 110,140" stroke="#5D4037" strokeWidth="8" strokeLinecap="round" fill="none" />
          <path d="M200,220 Q260,150 290,110" stroke="#5D4037" strokeWidth="6" strokeLinecap="round" fill="none" />
          {isWemby && (
            <path d="M200,180 Q300,100 350,50" stroke="#4CAF50" strokeWidth="8" strokeLinecap="round" fill="none" className="animate-pulse" />
          )}
        </g>
        ...
        <g ref={leavesRef} opacity="0">
          {/* Base leaves */}
          <circle cx="200" cy="180" r="80" fill={theme === 'silver' ? '#9CA3AF' : '#2E7D32'} opacity="0.9" />
          <circle cx="130" cy="230" r="60" fill={theme === 'silver' ? '#6B7280' : '#388E3C'} opacity="0.9" />
          <circle cx="270" cy="210" r="70" fill={theme === 'silver' ? '#4B5563' : '#1B5E20'} opacity="0.9" />
          
          {/* Mid leaves */}
          <circle cx="150" cy="130" r="65" fill={theme === 'silver' ? '#D1D5DB' : '#4CAF50'} opacity="0.9" />
          <circle cx="250" cy="140" r="75" fill={theme === 'silver' ? '#9CA3AF' : '#2E7D32'} opacity="0.9" />
          
          {/* High leaves (Stage 4+) */}
          {stageIndex >= 4 && (
            <>
              <circle cx="100" cy="180" r="70" fill={theme === 'silver' ? '#6B7280' : '#388E3C'} opacity="0.9" />
              <circle cx="300" cy="160" r="70" fill={theme === 'silver' ? '#4B5563' : '#1B5E20'} opacity="0.9" />
              <circle cx="200" cy="80" r="90" fill={theme === 'silver' ? '#D1D5DB' : '#4CAF50'} opacity="0.9" />
            </>
          )}
          ...
          {stageIndex >= 8 && (
            <>
              <circle cx="60" cy="140" r="80" fill={theme === 'silver' ? '#E5E7EB' : '#81C784'} opacity="0.8" />
              <circle cx="340" cy="120" r="80" fill={theme === 'silver' ? '#D1D5DB' : '#66BB6A'} opacity="0.8" />
              <circle cx="200" cy="20" r="110" fill={theme === 'silver' ? '#F3F4F6' : '#A5D6A7'} opacity="0.8" />
              <circle cx="120" cy="60" r="85" fill={theme === 'silver' ? '#D1D5DB' : '#4CAF50'} opacity="0.9" />
              <circle cx="280" cy="50" r="95" fill={theme === 'silver' ? '#6B7280' : '#388E3C'} opacity="0.9" />
            </>
          )}
      ...
      <AnimatePresence>
        {isJoker && (
          ...
          >
            <div className="text-green-500 font-black text-6xl md:text-8xl transform -rotate-12 opacity-90 drop-shadow-[0_0_15px_rgba(34,197,94,0.8)]" style={{ fontFamily: 'Impact, sans-serif' }}>
              HA HA HA
            </div>
          </motion.div>
        )}
...
const Background = ({ stageIndex, isSunbathing, isWhySoSerious, theme }: any) => {
  ...
  return (
    <div className={`absolute inset-0 transition-colors duration-1000 ${bgClass}`}>
      ...
      {/* Stars/Dust */}
      <div className="absolute inset-0 opacity-30" style={{ backgroundImage: 'radial-gradient(circle, white 1px, transparent 1px)', backgroundSize: '30px 30px' }} />
    </div>
  );
};
      ...
      <AnimatePresence>
        {particles.map(p => (
          <motion.div
            ...
            animate={{ opacity: 0, y: '0%', x: `${p.x + (Math.random() * 20 - 10)}%`, scale: 1.5 }}
            exit={{ opacity: 0 }}
...
export function WateringSystemModule() {
  ...
  const [stats, setStats] = useState({ water: 0, fertilize: 0, sunbathe: 0, prune: 0 });
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setStats(parsed.stats || { water: 0, fertilize: 0, sunbathe: 0, prune: 0 });
  ...
  // Save state
  useEffect(() => {
    localStorage.setItem('wateringSystemState', JSON.stringify({
      progress,
      stats,
      fertilizeUsesToday,
  ...
  const progressToNext = nextStage 
    ? ((progress - currentStage.threshold) / (nextStage.threshold - currentStage.threshold)) * 100 
    : 100;
  ...
  useEffect(() => {
    if ((currentStageIndex === 7 || currentStageIndex === 8) && containerRef.current) {
  ...
  const handleAction = (action: 'water' | 'fertilize' | 'sunbathe' | 'prune') => {
    let progressGain = 0;
    ...
    if (action === 'sunbathe') {
      ...
      showSpeech(REMARKS.sunbathe[Math.floor(Math.random() * REMARKS.sunbathe.length)]);
    }
    ...
    if (action === 'prune') {
      ...
      playAudio(1966);
      showSpeech(REMARKS.prune[Math.floor(Math.random() * REMARKS.prune.length)]);
    }
  ...
  return (
    ...
    >
      <Background stageIndex={currentStageIndex} isSunbathing={isSunbathing} isWhySoSerious={isWhySoSerious} theme={theme} />
      ...
      <div className="lg:col-span-2 flex flex-col items-center justify-between min-h-[500px] relative z-10">
        ...
        <div className="flex-1 w-full relative flex items-end justify-center perspective-1000 my-4">
          <TreeSVG stageIndex={currentStageIndex} isJoker={isJoker} isKnight={isKnight} isGotham={isGotham} isWemby={isWemby} theme={theme} />
            ...
            {showCelebration && (
              ...
              >
                <div className="bg-white/90 backdrop-blur-md px-8 py-4 rounded-3xl border-4 border-green-500 shadow-[0_0_50px_rgba(34,197,94,0.5)] text-center">
                  <h2 className="text-3xl font-black text-green-600 uppercase tracking-widest mb-2">Level Up!</h2>
                  <p className="text-xl font-bold text-gray-800">{celebrationText}</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
      ...
      <div className="lg:col-span-1 border-t lg:border-t-0 lg:border-l border-white/20 pt-6 lg:pt-0 lg:pl-6 flex flex-col z-10">
        <h4 className="font-mono text-xs uppercase tracking-widest font-bold text-white mb-4 drop-shadow-md">Growth Stats</h4>
        ...
        <div className="space-y-3 mb-6">
          <div className="flex justify-between items-center p-3 bg-white/10 backdrop-blur-sm rounded-3xl border border-white/20 text-white">
            <span className="text-sm font-bold flex items-center gap-2"><Droplets size={14} className="text-blue-300"/> Watered</span>
            <span className="font-mono font-black">{stats.water}</span>
          </div>
          <div className="flex justify-between items-center p-3 bg-white/10 backdrop-blur-sm rounded-3xl border border-white/20 text-white">
            <span className="text-sm font-bold flex items-center gap-2"><Sparkles size={14} className="text-purple-300"/> Fertilized</span>
            <span className="font-mono font-black">{stats.fertilize}</span>
          </div>
          <div className="flex justify-between items-center p-3 bg-white/10 backdrop-blur-sm rounded-3xl border border-white/20 text-white">
            <span className="text-sm font-bold flex items-center gap-2"><Sun size={14} className="text-orange-300"/> Sunbathed</span>
            <span className="font-mono font-black">{stats.sunbathe}</span>
          </div>
          <div className="flex justify-between items-center p-3 bg-white/10 backdrop-blur-sm rounded-3xl border border-white/20 text-white">
            <span className="text-sm font-bold flex items-center gap-2"><Scissors size={14} className="text-green-300"/> Pruned</span>
        ...
        <div className="flex-1 bg-black/40 backdrop-blur-md rounded-3xl p-4 border border-white/20 flex flex-col justify-center items-center text-center text-white relative overflow-hidden">
              ...
              {['classic', 'pvz', 'cyber', 'silver'].map(t => (
                <button
                  ...
                  className={`w-4 h-4 rounded-full border border-white/50 ${theme === t ? 'ring-2 ring-white scale-110' : 'opacity-50'} ${
                    t === 'classic' ? 'bg-green-500' : 
                    t === 'pvz' ? 'bg-lime-400' : 
                    t === 'cyber' ? 'bg-cyan-400' : 'bg-gray-300'
                  }`}
                  title={`Theme: ${t}`}
                />
              ))}
          ...
          <div className="font-black uppercase tracking-tight text-2xl leading-none mb-2">{currentStage.name}</div>
          <div className="text-xs font-mono text-white/60 uppercase mb-4">Stage {currentStageIndex + 1} of 9</div>
