...
export const PhysicsStack: React.FC<PhysicsStackProps> = ({ temperature, onIconClick, techData }) => {
  ...
  useEffect(() => {
    ...
    Events.on(render, 'afterRender', () => {
      ...
      engine.world.bodies.forEach((body) => {
        ...
        if (item) {
          ...
          // Draw text
          const text = item.name.length > 8 ? item.name.substring(0, 6) + '..' : item.name;
    ...
    Events.on(engine, 'beforeUpdate', () => {
      ...
      if (temperatureRef.current < 100) { // Disable magnetic attraction if temperature is max (chaos)
  ...
  return (
    <div className="w-full h-full relative overflow-hidden rounded-3xl" ref={sceneRef}>
      {/* Noise Background */}
      <div 
