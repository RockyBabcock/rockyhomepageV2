  ...
  const blogs = [
    {
      ...
      excerpt: "Exploring why our eyes are becoming fatigued by the standard UI boxes and how natural depth through color shifts is the next frontier of digital tactility..."
    },
    {
      ...
      excerpt: "The tactile quality of newsprint and high-end editorial magazines can be translated to the screen through careful attention to ink-traps and bold weights..."
    }
  ];
    ...
    >
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-4">
        <h2 className="text-4xl md:text-5xl font-headline font-black leading-none">
          Dispatches <br/> from the <span className="italic text-primary">Grid.</span>
        </h2>
        <Link to="/blog" className="font-label text-xs uppercase tracking-widest border-b-2 border-primary text-primary pb-1 font-black hover:tracking-[0.2em] transition-all">
          View Archive
        </Link>
      </div>
      ...
      <div className="space-y-12 md:space-y-16">
        {blogs.map((blog) => (
          <Link to={`/blog/${blog.slug}`} key={blog.slug} className="block group cursor-pointer border border-transparent hover:border-ink/5 dark:hover:border-base/5 p-4 -m-4 rounded-2xl transition-all">
