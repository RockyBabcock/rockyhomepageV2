...
export function ThoughtStarMap() {
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    setIsDragging(true);
    dragStartRef.current = { x: e.clientX, y: e.clientY, rotH: rotationH, rotV: rotationV };
    ...
    <div className="w-full h-full relative overflow-hidden flex bg-[#050505]">
      ...
      >
        {points.map((p) => {
            ...
            const opacity = activeQuoteId ? (p.id === activeQuoteId ? 1 : 0.1) : Math.max(0.1, scale * 1.5 - 0.5);
            ...
            return (
              <motion.div
                ...
                className={cn(
                  ...
                  activeQuoteId === p.id ? "pointer-events-none" : "cursor-pointer"
                )}
                style={{
                  x: px,
                  y: py,
                ...
                onMouseLeave={() => activeQuoteId === p.id && setActiveQuoteId(null)}
              >
                  <div className={cn(
                    "w-2 h-2 rounded-full transition-all duration-500",
                    activeQuoteId === p.id ? "bg-[#d4af37] shadow-[0_0_15px_#d4af37]" : "bg-white/50"
                  )} />
                  <span className={cn(
      ...
      <AnimatePresence>
        {activeQuote && (
          ...
          >
            <span className="font-mono text-[10px] uppercase tracking-widest text-[#d4af37] mb-4">Constellation: {activeQuote.category}</span>
            <h3 className="font-headline font-black text-2xl text-white mb-6">"{activeQuote.text}"</h3>
            <p className="font-mono text-xs text-white/50 mb-12">— {activeQuote.author}</p>
            ...
            <div className="border-l-2 border-[#d4af37]/50 pl-6 mb-8 relative pointer-events-auto">
              ...
              <span className="block text-[9px] uppercase font-bold tracking-widest opacity-40 mb-3 text-white">System Mapping</span>
            ...
            <button className="flex items-center gap-2 font-mono text-[10px] uppercase font-bold tracking-widest text-[#d4af37] hover:text-white transition-colors mt-auto pointer-events-auto group w-max">
              View Design Case <ChevronRight className="w-3 h-3 group-hover:translate-x-1 transition-transform" />
            </button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 z-40">
         <span className="font-mono text-[9px] uppercase tracking-widest text-[#d4af37]">Time Distortion Field</span>
            ...
            {[
              ...
              >
                <div className={cn(
                  ...
                  timePeriod >= t.id ? "bg-[#d4af37] shadow-[0_0_10px_#d4af37]" : "bg-white/20 group-hover:bg-white/40"
                )} />
                <span className={cn(
