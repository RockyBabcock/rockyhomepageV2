...
export function InfiniteChessboard() {
  ...
  useEffect(() => {
    ...
    const typeSymbols: Record<string, string> = { pawn: '♙', knight: '♘', bishop: '♗', rook: '♖', queen: '♕', king: '♔' };
    ...
    const initPieces = () => {
      ...
      for (let i = 0; i < numPieces; i++) {
        ...
        pieces.push({
          ...
          type,
          size: type === 'king' || type === 'queen' ? 30 : Math.random() * 10 + 15,
    ...
    const handleKeyDown = (e: KeyboardEvent) => { if (e.code === 'Space') timeScale = 4; };
    const handleKeyUp = (e: KeyboardEvent) => { if (e.code === 'Space') timeScale = 1; };
    ...
    const draw = () => {
      ...
      ctx.fillStyle = mode === 'flow' ? `rgba(5, 5, 5, ${timeScale === 1 ? 0.3 : 0.05})` : 'rgba(5, 5, 5, 0.05)';
          ...
          if (p.type === 'pawn') {
            ...
            if (Math.abs(p.vx) !== Math.abs(p.vy)) {
             ...
             if (Math.random() < 0.01 * tm) {
               if (Math.random() > 0.5) { p.vx = (Math.random() > 0.5 ? 2 : -2); p.vy = 0; }
               else { p.vx = 0; p.vy = (Math.random() > 0.5 ? 2 : -2); }
             }
             p.x += p.vx * tm; p.y += p.vy * tm;
             if (p.x < 0 || p.x > width) p.vx *= -1;
             if (p.y < 0 || p.y > height) p.vy *= -1;
          } else if (p.type === 'king' || p.type === 'queen') {
             const centerX = width / 2;
             const centerY = height / 2;
             p.x += (centerX - p.x) * 0.001 * tm + (Math.random() - 0.5) * 0.5 * tm;
             p.y += (centerY - p.y) * 0.001 * tm + (Math.random() - 0.5) * 0.5 * tm;
          }
  ...
  return (
    <div ref={containerRef} className="w-full h-full relative bg-[#050505] overflow-hidden">
      ...
      <div className="absolute top-12 left-1/2 -translate-x-1/2 text-center pointer-events-none z-10 w-full">
        <h2 className="font-headline font-black text-xl uppercase tracking-[0.2em] text-white opacity-80 mix-blend-overlay">The Infinite Board</h2>
        <p className="font-mono text-[9px] uppercase tracking-widest text-[#d4af37] opacity-60 mt-2">Emergent architecture through systemic movement</p>
        <p className="font-mono text-[9px] uppercase tracking-widest text-white/40 mt-4 animate-pulse">Hold [Space] to warp time</p>
      </div>
    </div>
  );
}
